...
def do_train(cfg, model, resume=False):
    ...
    with EventStorage(start_iter) as storage:
        ...
        for data1, data2, iteration in zip(data_loaders[0], data_loaders[1], range(start_iter, max_iter)):
            ...
            # print(f"Toal Loss:{losses}")
            # print(f"Toal Loss:{loss_dict}")
...
def setup(args):
    ...
    cfg.DATASETS.TRAIN_DIR_LIST = [("train_visdrone_s","train_visdrone_tp"),("train_uavdt_t","train_uavdt_sp")]
    cfg.DATASETS.TRAIN_ROOT_LIST = ["./VisDrone2019", "./UAVDT"]
    cfg.DATASETS.TRAIN_LIST = ["visdrone_S_TP_train","uavdt_T_SP_train"]
    cfg.DATASETS.TEST = ()
    ...
    cfg.SOLVER.MAX_ITER = 62500  # 300 iterations seems good enough for this toy dataset; you will need to train longer for a practical dataset
    cfg.SOLVER.CONTRASTIVE_START_ITER = 0 #cfg.SOLVER.MAX_ITER + 10 #int(cfg.SOLVER.MAX_ITER / 2)
    cfg.SOLVER.STEPS = [25000, 50000]        # do not decay learning rate
    ...
    cfg.OUTPUT_DIR=cfg.OUTPUT_DIR+"./work_dir" #ResNet101_MLP_layer_constrastive_learning R50BackL0calGlobal_UAV
    cfg.SAVE_H5= False
...
def get_dicts(img_dirs, rootpath, skip = False):
    json_file = os.path.join(rootpath, "annotations.json")
    ...
    if skip:
        directory_path = "./UAVDT/train_uavdt_t"

        # list all files in the directory
        files = os.listdir(directory_path)
        # print(f"Number of images in UAVDT: {len(files)}")
...
def registrar_dataset_visdrone(cfg):
        ...
        MetadataCatalog.get(cfg.DATASETS.TRAIN_LIST[0]).set(thing_classes=['bycicyle', 'car', 'van', 'truck', 'bus','motor'])

def registrar_dataset_uavdt(cfg):
...
def main(args):
    ...
    cfg = setup(args)
    #print(cfg)
    registrar_dataset_visdrone(cfg)

    registrar_dataset_uavdt(cfg)
