...
class CustomROIHeads(StandardROIHeads):
    ...
    def forward(self, images, features, proposals, unknown_proposals, rand_idx, unknown_index, name, num_iter, targets=None):
        """
        enable debug
        """
        #print("The len of the proposal: ", len(proposals))
        # print("The len of the name: ", len(name))

        #print("####### Inside CustomROIHeads#######", features.shape)
        ...
        if self.training:
            ...
            proposals = self.label_and_sample_proposals(proposals, targets) #Per image proposals after matching with GT. Shape is( 256/Batch size per image) , 
            #                                                               Fields: num_instances, img_h, img_w, proposals_boxes, objectness_logits, gt classes

            for proposal in proposals:
                gt_classes=proposal.get("gt_classes").cpu().detach().numpy()
                #print("GT: ",gt_classes)
                ptr=0
                for class_id in gt_classes:
                    if class_id!=10:
                        ptr+=1
                object_per_image.append(ptr)


        del targets

        if self.training:
            losses  = self._forward_box(features, proposals, object_per_image, unknown_proposals, rand_idx, unknown_index, name, num_iter)
            losses.update(self._forward_mask(features, proposals))
            losses.update(self._forward_keypoint(features, proposals))
            return proposals, losses
        else:
            pred_instances = self._forward_box(features, proposals, object_per_image, None, None, None, name, num_iter)
