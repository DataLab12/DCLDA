...
class CenterNet(nn.Module):
    ...
    def __init__(self, 
        ...
        *,
        ...
        self.post_nms_topk_targetadap = post_nms_topk_targetadap
        ...
        self.cnt=0
        #print("################# Inside CenterNet HEAD Proposal Generator #####################")
    @classmethod
    def from_config(cls, cfg, input_shape):
        #print("The input shape: ",input_shape)
        #print("COnfig:::::::::::::::", cfg.MODEL.CENTERNET.IN_FEATURES)
        ret = {
            ...
            'post_nms_topk_targetadap': cfg.MODEL.CENTERNET.POST_NMS_TOPK_TARGETADAP,
    ...
    def forward(self, images, target_images, rand_idx, unknown_index, features_dict, gt_instances):


        # only_src_len=  len(gt_instances)
        if not self.training:
            features = [features_dict[f] for f in self.in_features]
        else:  
            features = [features_dict[f][rand_idx] for f in self.in_features]

        clss_per_level, reg_pred_per_level, agn_hm_pred_per_level = \
            self.centernet_head(features)

        grids = self.compute_grids(features)
        #print("Grid 0 shape: ",grids[0].shape)
        shapes_per_level = grids[0].new_tensor(
                    [(x.shape[2], x.shape[3]) for x in reg_pred_per_level])
        
        #print("Shapes per level: \n", shapes_per_level)
        if not self.training:
            return self.inference(
                images, clss_per_level, reg_pred_per_level, 
                agn_hm_pred_per_level, grids)
        else:
            unknown_features = [features_dict[f][unknown_index] for f in self.in_features]
            unknown_proposals= self._get_unknown_proposals(unknown_features, target_images)
            ...
            if self.only_proposal:
                # print("Only proposals")
                agn_hm_pred_per_level = [x.sigmoid() for x in agn_hm_pred_per_level]
                # print("Sigmoid output agn_hm in Centernet: ",agn_hm_pred_per_level[0].shape)
                # print("Sigmoid output agn_hm in Centernet: ",agn_hm_pred_per_level[4][0])
                ...
                # print(len(proposals[0].get('pred_boxes')))
            ...
            if self.only_proposal or self.as_proposal:
                # print("Len of the proposals: ", len(proposals))
                for p in range(len(proposals)):
                    # print("!!!!!!!!!!    ", p)
                    # print(len(proposals[p].get('pred_boxes')))
                    # print(len(proposals[p].get('scores')))
                    # print("!!!!!!!!!!!!!!!!!!")
            ...
            #print("The value of debug: ",self.debug)
            if self.debug:
                debug_train(
            ...
            # print(proposals[0].get('proposal_boxes')[0])

            return proposals, unknown_proposals, losses
    ...
    def compute_grids(self, features):
        ...
        for level, feature in enumerate(features):
            ...
            # print("Strides: ",self.strides[level] // 2)
            # print("Height: ",h,"    ","width: ",w)
            shifts_x = torch.arange(
                0, w * self.strides[level], 
                step=self.strides[level],
                dtype=torch.float32, device=feature.device)
            # print("X shifts:", shifts_x)
            shifts_y = torch.arange(
                0, h * self.strides[level], 
                step=self.strides[level],
                dtype=torch.float32, device=feature.device)
            # print("Shifts Y: ", shifts_y)
            shift_y, shift_x = torch.meshgrid(shifts_y, shifts_x)
            # print("Shifts X: ", shift_x)
            # print("Shifts Y: ", shift_y)
            shift_x = shift_x.reshape(-1)
            shift_y = shift_y.reshape(-1)
            # print("Shifts X Flatten: ",shift_x.shape)
            # print("Shifts Y Flatten: ",shift_y.shape)

            temp=torch.stack((shift_x, shift_y), dim=1)

            # print("Matrix after stacking : ", temp)
            grids_per_level = torch.stack((shift_x, shift_y), dim=1) + self.strides[level] // 2

            #print("Grids per level: ",grids_per_level)
            grids.append(grids_per_level)
        return grids

    def _get_unknown_proposals(self, features, images):
            clss_per_level, reg_pred_per_level, agn_hm_pred_per_level = \
                self.centernet_head(features)
            ...
            proposals = self.predict_instances(
                    grids, agn_hm_pred_per_level, reg_pred_per_level, 
                    images.image_sizes, [None for _ in agn_hm_pred_per_level], domainadap= True)
    ...
    def _get_label_inds(self, gt_instances, shapes_per_level):
        ...
        for im_i in range(B):
            ...
            #print("HEEEEEEEEE: ",is_cared_in_the_level.shape)
    ...
    def assign_fpn_level(self, boxes):
        ...
        #print("Size of interest: ", self.sizes_of_interest)
    ...
    def inference(self, images, clss_per_level, reg_pred_per_level, 
        ...
        # print("Score thers: ", self.score_thresh)
        if self.only_proposal:
            # print("inside only proposal at inference")
            proposals = self.predict_instances(
                grids, agn_hm_pred_per_level, reg_pred_per_level, 
                images.image_sizes, [None for _ in agn_hm_pred_per_level])
            # print("Length of the proposal:{}  and shape: {}".format(len(proposals), proposals[0]))
        else:
            proposals = self.predict_instances(
                grids, logits_pred, reg_pred_per_level, 
                images.image_sizes, agn_hm_pred_per_level)
        if self.as_proposal or self.only_proposal:
            # print("Vhul hoye gese!!!!!!!")
            for p in range(len(proposals)):
                proposals[p].proposal_boxes = proposals[p].get('pred_boxes')
                proposals[p].objectness_logits = proposals[p].get('scores')
                proposals[p].remove('pred_boxes')
    ...
    def predict_instances(
        ...
        is_proposal=False, domainadap=False):
        ...
        boxlists = self.nms_and_topK(
            boxlists, domainadap, nms=not self.not_nms)
        return boxlists
    ...
    def predict_single_level(
        ...
        # print("The final shape of the heatmap: ",heatmap.shape)
        ...
        for i in range(N):
            ...
            # print("Lenght of per class: ", len(per_class))
    ...
    @torch.no_grad()
    def nms_and_topK(self, boxlists, domainadap= False, nms=True):
        #print("What is the value of nms: ", nms)
        num_images = len(boxlists)
        results = []
        for i in range(num_images):
            nms_thresh = self.nms_thresh_train if self.training else \
                self.nms_thresh_test
            #print("NMS thres: ", nms_thresh)
            ...
            num_dets = len(result)
            #print("The number of detections: ", num_dets)
            post_nms_topk = self.post_nms_topk_train if self.training else \
                self.post_nms_topk_test

            if domainadap:
                post_nms_topk = self.post_nms_topk_targetadap

            if num_dets > post_nms_topk:
                cls_scores = result.scores
                #print("The class scores shape: ",cls_scores.shape)
                image_thresh, _ = torch.kthvalue(
                    cls_scores.float().cpu(),
                    num_dets - post_nms_topk + 1
                )
                # if domainadap:
                #     print(f"Post topk: {post_nms_topk}")
                #     print(f"Img thres: {image_thresh.item()}")
                keep = cls_scores >= image_thresh.item()
                keep = torch.nonzero(keep).squeeze(1)
                result = result[keep]
                # print(f"Boxes: {result}")
            if self.debug:
