...
class CenterNetHead(nn.Module):
    ...
    def __init__(self, 
        ...
        *,
        ...
        # print("Number of channels: ",in_channels)
        channels = {
            'cls': in_channels,
            'bbox': in_channels,
            'share': in_channels,
        ...
        print("Head Config: ",head_configs)
        ...
        for head in head_configs:
            ...
            print(f"Head: {head} and Conv: {num_convs}")
            for i in range(num_convs):
                print("Value of num_conv {}".format(i))
        ...
        if self.with_agn_hm:
            ...
            #print("Shape agn hm bias: ",self.agn_hm.bias.shape)
            #print("Shape agn hm weight: ",self.agn_hm.weight.shape)
    ...
    def forward(self, x):
        ...
        for l, feature in enumerate(x):
            ...
            # print(f"Here feature: {feature.shape}")
            feature = self.share_tower(feature)
            cls_tower = self.cls_tower(feature)
            bbox_tower = self.bbox_tower(feature)

            # for x in range(bbox_tower.size()[0]):
            #     f_map={}
            #     f1=feature[x]
            #     f= bbox_tower[x]
            #     #print("what is the size: ", f1.shape)
            #     gray_scale = torch.sum(f1,0)
            #     #print("Size after sum: ", gray_scale.shape)
            #     gray_scale = gray_scale / f1.shape[0]
            #     f_map['hmap']=gray_scale.data.cpu().numpy()
            #     f_map['name']=name[x]
            #     processed.append(f_map)

            # print("Output from bbox tower: ", bbox_tower.shape)
            if not self.only_proposal:
                clss.append(self.cls_logits(cls_tower))
            else:
                clss.append(None)
            #print("Class outcome: ",clss[l])
            if self.with_agn_hm:
                agn_hms.append(self.agn_hm(bbox_tower))
            else:
                agn_hms.append(None)

            # print(f"Layer:{l} and Outcome from agn_hm: {agn_hms[l].shape}")
            #print(agn_hms[l][0])

            reg = self.bbox_pred(bbox_tower)

            #print("Outcome bbox_pred: ", reg.shape)
            reg = self.scales[l](reg)

            #print("Outcome of bbox after scaling: ",reg.shape)
            bbox_reg.append(F.relu(reg))
            #print("Outcome of bbox after relu: ",bbox_reg[l].shape)
